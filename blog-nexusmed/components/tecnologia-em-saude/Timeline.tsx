const EVENTOS = [
  ...
  },
  {
    ...
    descricao: 'A ANPD inicia aplicação de sanções. Clínicas que não adequaram seus sistemas de prontuário e coleta de dados ficaram expostas a riscos legais.',
    icon: '\u{1F510}',
    color: '#DC2626',
    bg: '#FEE2E2',
  },
      ...
      <div className="space-y-6">
        {EVENTOS.map((ev) => (
          <div key={ev.ano} className={`flex gap-5 md:gap-8 items-start ${ev.destaque ? 'relative' : ''}`}>
            {/* Bola */}
            <div
              className="flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center text-xl z-10 shadow-md"
              style={{ background: ev.bg, border: `2px solid ${ev.color}` }}
            >
              {ev.icon}
            </div>
            {/* Conteúdo */}
            <div
              className="nexus-card flex-1 p-5"
              style={ev.destaque ? { boxShadow: '0 0 0 2px var(--nexus-blue)' } : {}}
            >
              <div className="flex items-center gap-3 mb-2">
                <span
                  className="text-xs font-bold px-3 py-1 rounded-full"
                  style={{ background: ev.bg, color: ev.color, fontFamily: 'Poppins, sans-serif' }}
                >
                  {ev.ano}
                </span>
                {ev.destaque && (
